	...
	var DeviceRow = _wrapComponent('DeviceRow')(function (_React$Component) {
	    ...
	    _createClass(DeviceRow, [{
	        ...
	        value: function render() {
	            try {
	                status = typeof this.props.device.notes[0].status !== 'undefined' ? true : false;
	                if (status) {
	                    console.log(this.props.device.notes[0].status);
	                }
	            } catch (err) {
	                alert("status is undefined: " + err);
	            }

	            if (typeof this.props.device.notes[0].status === "undefined") {
	                alert("status");
	            } else {
	                console.log(this.props.device.notes[0].status);
	            }
